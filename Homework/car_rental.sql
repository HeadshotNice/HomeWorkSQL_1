...
CREATE TABLE rental (
  ...
  planned_period TSRANGE NOT NULL,
  actual_period TSRANGE NULL,
...
CREATE TABLE maintenance (
  ...
  service_period TSRANGE NOT NULL,
...
AS $$
BEGIN
  -- Only active-ish rentals participate in the check
  IF NEW.status_code IN ('NEW', 'ACT') THEN
    IF EXISTS (
      SELECT 1
      FROM maintenance m
...
CREATE TRIGGER rental_car_active_bi
...
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA car_rental TO cr_agent;
...
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA car_rental TO cr_accountant;
...
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA car_rental TO cr_mechanic;
